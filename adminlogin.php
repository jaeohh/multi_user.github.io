    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $admin['password'])) {
            if ($admin['status'] === 'inactive') {
                ...
                $conn->query("UPDATE admin SET verification_code = '$verification_code' WHERE email = '$email'");
                ...
                try {
                    ...
                    $mail->Body = "<p>Hello <strong>{$admin['first_name']}</strong>,</p>
                    ...
                    $mail->send();
                    $_SESSION['admin_email_for_verification'] = $email;
                } catch (Exception $e) {
...
if (isset($_POST['verify_email'])) {
    ...
    $email = $_SESSION['admin_email_for_verification'];

    $result = $conn->query("SELECT * FROM admin WHERE email = '$email' AND verification_code = '$verification_code'");
    if ($result->num_rows > 0) {
        $conn->query("UPDATE admin SET is_verified = 1, status = 'active' WHERE email = '$email'");
        unset($_SESSION['admin_email_for_verification']);

        $adminData = $conn->query("SELECT * FROM admin WHERE email = '$email'")->fetch_assoc();
...
<?php if (isset($_SESSION['admin_email_for_verification'])): ?>
