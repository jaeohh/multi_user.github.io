...
if (isset($_POST['add_teacher'])) {
    ...
    mysqli_query($conn, "INSERT INTO teachers (first_name, last_name, email, password, verification_code, is_verified, status) 
...
if (isset($_POST['update_teacher'])) {
    ...
    if (!empty($_POST['password'])) {
        ...
        $getPassword = mysqli_fetch_assoc(mysqli_query($conn, "SELECT password FROM teachers WHERE teacher_id='$id'"));
...
if (isset($_GET['toggle_status'])) {
    ...
    $get = mysqli_fetch_assoc(mysqli_query($conn, "SELECT status FROM teachers WHERE teacher_id='$id'"));
    $newStatus = ($get['status'] == 'active') ? 'inactive' : 'active';
    mysqli_query($conn, "UPDATE teachers SET status='$newStatus' WHERE teacher_id='$id'");
}
...
<body class="bg-gray-100 min-h-screen p-6">
  <div class="container mx-auto bg-white p-6 rounded shadow">
    <div class="flex justify-between items-center mb-4">
      ...
      <button onclick="openAddModal()" class="bg-green-600 text-white px-4 py-2 rounded">+ Add Teacher</button>
    </div>
      ...
      <tbody>
        ...
        <tr class="text-center">
          ...
          <td class="py-2 px-4 border">
            <a href="?toggle_status=<?php echo $row['teacher_id']; ?>" class="px-2 py-1 text-white rounded <?php echo $row['status'] == 'active' ? 'bg-green-500' : 'bg-red-500'; ?>">
          ...
          <td class="py-2 px-4 border space-x-2">
            ...
            <a href="?delete=<?php echo $row['teacher_id']; ?>" class="bg-red-600 text-white px-2 py-1 rounded" onclick="return confirm('Delete this teacher?')">Delete</a>
          </td>
        </tr>
        <?php endwhile; ?>
      </tbody>
    ...
    <div class="bg-white p-6 rounded shadow-lg w-full max-w-lg">
      ...
      <form method="POST">
        ...
        <div class="flex justify-end space-x-2">
          ...
          <button type="submit" name="add_teacher" class="px-4 py-2 bg-green-600 text-white rounded">Add</button>
        </div>
      </form>
    </div>
  </div>
    ...
    <div class="bg-white p-6 rounded shadow-lg w-full max-w-lg">
      ...
      <form method="POST">
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
          ...
          <input type="text" name="password" id="edit_password" placeholder="Password" class="p-2 border rounded" required>
        ...
        <div class="flex justify-end space-x-2">
          ...
          <button type="submit" name="update_teacher" class="px-4 py-2 bg-blue-600 text-white rounded">Update</button>
        </div>
      </form>
    </div>
  </div>
